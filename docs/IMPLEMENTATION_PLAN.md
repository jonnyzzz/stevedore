...
2. **Git sync** (`stevedore deploy sync`) — isolated git worker containers
   - Runs Git operations inside an `alpine/git` worker container with the generated SSH key
   - Stores checkout under `deployments/<name>/repo/git/`
   - Returns commit hash and branch info
   - Worker-container implementation lives in `internal/stevedore/git_worker.go`
...
6. **CI**
   - Integration tests run in Docker (driven by Go integration tests: `go test ./tests/integration/...`).
...
- Git operations run in worker containers by default.
